...
%% Build:  make compile  (needs figs/, tables/ and snippets/, all written by
%%                        make assets, which runs code/paper.py)
%%
%% Plain article: the note is not a SIAM paper and should not wear the SIAM
%% journal class.  Everything siamltex supplied and article does not is
%% rebuilt below, with no package beyond the three loaded here, so the source
%% compiles on a bare TeX Live -- which is what arXiv gives it.
...
% A text block narrower than article's default.  Plain lengths rather than
% geometry, which is not in the minimal TeX Live the CI installs.
...
\makeatletter
% Run-in section numbers -- "1. Introduction", and "Appendix A. A sixth
% problem" once \appendix has been seen.  The prefix is looked up per level
% with \csname rather than tested with \ifx, because \@sect puts this through
% a \protected@edef, where a \def in the body would come apart.  Only the
% section level carries a prefix, so subsections stay "A.1".
\newcommand\@seccntpre@section{}
\newcommand\@seccntpre@subsection{}
\renewcommand\@seccntformat[1]{%
  \csname @seccntpre@#1\endcsname\csname the#1\endcsname.\hskip 0.5em\relax}
\g@addto@macro\appendix{\renewcommand\@seccntpre@section{\appendixname~}}
\makeatother

% The four environments the note uses that article does not define.
\newtheorem{lemma}{Lemma}[section]
\newenvironment{proof}
  {\par\noindent\textit{Proof.}\ \ignorespaces}
  {\hfill$\square$\par\medskip}
\newenvironment{keywords}
  {\par\medskip\noindent\small\textbf{Key words.}\ \ignorespaces}
  {\par}
\newenvironment{AMS}
  {\par\smallskip\noindent\small\textbf{AMS subject classifications.}\ %
   \ignorespaces}
  {\par\medskip}
...
\texttt{code/}, and \texttt{make assets} reproduces every number below.}}
...
\begin{keywords}
high-accuracy computation, Markov chain, elliptic billiard, Rouch\'e's
theorem, Taylor-series integration, elastic collision, contour shift,
reproducible computation
\end{keywords}

\begin{AMS}
00A08, 30E20, 60J10, 65D30, 65F15, 65L05
\end{AMS}
...
The MATLAB and Mathematica of 2005 has been rewritten in Python: Claude
Opus 5 was asked to revisit the original article and redo its experiments.
The listings in this note are extracted from the files that produce its
numbers, so the two cannot drift apart, and every figure, table and
displayed constant comes out of \texttt{make assets}. I have also fixed the
typography and the typing, of which there was a good deal.
...
\begin{figure}[htb]
\centerline{\includegraphics[width=0.42\textwidth]{figs/board}}
...
\begin{figure}[htb]
\centerline{\includegraphics[width=0.46\textwidth]{figs/spy}}
...
\input{snippets/photon-reflect}

The flight is then a loop over events:

\input{snippets/photon-fly}

\begin{figure}[htb]
\centerline{\includegraphics[width=0.58\textwidth]{figs/mirrors}}
...
\begin{figure}[htb]
\centerline{\includegraphics[width=0.52\textwidth]{figs/spectrum}}
...
\input{snippets/planets-integrate}

\begin{figure}[htb]
\centerline{\includegraphics[width=0.56\textwidth]{figs/orbits}}
...
\input{snippets/bowl-run}

\begin{figure}[htb]
\centerline{\includegraphics[width=0.5\textwidth]{figs/fishbowl}}
...
\texttt{make assets}, at two precisions or by two methods.

\begin{thebibliography}{9}
\bibitem{bornemann} F. Bornemann, D. Laurie, S. Wagon and J. Waldvogel,
\emph{The SIAM 100-Digit Challenge: A Study in High-Accuracy Numerical
Computing}, SIAM, Philadelphia, 2004.
\bibitem{hydra} F. Bornemann and T. Schmelzer, \emph{Taming a hydra of
singularities}, Amer. Math. Monthly \textbf{114} (2007), no.~8, 727--732.
...
plane gives us everything. The argument sketched here was afterwards worked
out with Folkmar Bornemann and published in the Monthly \cite{hydra}, where
the integrand is one member of a family and the closed form below is one
value of a limit.
...
\begin{figure}[htb]
\centerline{\includegraphics[width=0.5\textwidth]{figs/contour}}
...
\input{snippets/integral-closed}
...
The same contour shift handles $\sin^2$ of any number of nested tangents,
and the values approach a limit that \cite{hydra} identifies; the problem
above is the second member of that family.
